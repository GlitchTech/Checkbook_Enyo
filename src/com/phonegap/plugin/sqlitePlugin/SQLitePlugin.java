 ...
 * 
 * Copyright (c) 2005-2010, Nitobi Software Inc. Copyright (c) 2010, IBM Corporation
 */
		...
		try {
			...
			for( int i = 0; i < len; i++ ) {
				...
				if( query.toLowerCase().startsWith( "insert" ) && jsonparams != null ) {
					...
					if( query_id.length() > 0 ) this.processResults( myCursor, query_id, tx_id );
			...
			this.sendJavascriptCB( "window.SQLitePluginTransactionCB.queryErrorCallback('" + tx_id + "','" + query_id + "', '" + ex.getMessage() + "');" );
